...
export class ProfessionalComponent implements OnInit {
  ...
  professionalphoto: any = ['assets/images/basket.png'];
  base64: string;
  userid: any;
 ...
 countryArr :any= [
  ...
  eachprofessionalData: any ={professionalName:'',email:'',countryCode:'',mobileNumber:'',status:''};
  ...
  ngOnInit() {
    ...
    this.formvalidation();
    this.professionalApiFunc('',this.filter.currPage);
  }
formvalidation(){
  this.addprofessionalForm = new FormGroup ({ 
    image:new FormControl('',Validators.required),
    professionalname: new FormControl('', [Validators.required ,Validators.pattern(/^[a-zA-Z\s]*$/)]),
     email: new FormControl('',[Validators.required,Validators.pattern(/^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,3})$/)]),
     mobilecode: new FormControl('', [Validators.required,Validators.pattern(/^[0-9\s]*$/)]),
  ...
  this.editprofessionalForm = new FormGroup ({ 
    image:new FormControl('',Validators.required),
    professionalname: new FormControl('', [Validators.required ,Validators.pattern(/^[a-zA-Z\s]*$/)]),
     email: new FormControl('',[Validators.required,Validators.pattern(/^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,3})$/)]),
     mobilecode: new FormControl('', [Validators.required,Validators.pattern(/^[0-9\s]*$/)]),
...
handleReaderLoaded(e) {
  this.professionalphoto.push('data:image/png;base64,' + btoa(e.target.result)); 
  this.base64 = 'data:image/png;base64,' + btoa(e.target.result) ;
}
/*********************** Image Conversion/***********************/
/*********************** Image Conversion/***********************/
editChange(evt: any) {
  this.editprofessionalphoto = [] 
  // this.service.spinnerShow();
  const file = evt.target.files[0];
...
editReaderLoaded(e) {
  this.editprofessionalphoto.push('data:image/png;base64,' + btoa(e.target.result)); 
  this.edit64 = 'data:image/png;base64,' + btoa(e.target.result) ;
}
/*********************** Image Conversion/***********************/
/********** Add Professional Data ***************/
addprofessionalFunc(formval){
  console.log("Form Val---> ",formval);
  var profData = {
    "organizerId":this.userid._id,
    "professionalName":formval.professionalname,
     "email":formval.email,
    "countryCode":formval.mobilecode,
    "mobileNumber":formval.mobile,
    "status":formval.status,
    "imageURL":this.base64
    }
    console.log("ApiData" , JSON.stringify(profData));
    this.service.postApi(`membership/addProfessional`,profData,1).subscribe(response => {
      if(response.responseCode == 201) {
        console.log(JSON.stringify(response));
        this.addprofessionalForm.reset();
        this.professionalphoto = ['assets/images/basket.png'];
        $('#create_professional').modal('hide');
        this.professionalApiFunc('',this.filter.currPage);
...
professionalApiFunc(formval,page){
  this.filter.currPage = page;
  console.log("Form Val---> ",formval," PageNo--> ",page);
  var profData = {
    "page":this.filter.currPage,
    "limit":this.filter.limit,
     "search":formval.search?formval.search:"",
       }
    console.log("ApiData--> " , JSON.stringify(profData));
    var url=`membership/getListOfProfessional?organizerId=`+this.userid._id;
    this.service.postApi(url,profData,1).subscribe(response => {
      if(response.responseCode == 200) {
        console.log(JSON.stringify(response));
        this.professiondoc = response.result;
        this.pageLimit = this.professiondoc.limit;
        this.pageTotal = this.professiondoc.total;
        this.professionalList = this.professiondoc.docs;
        } else if(response.responseCode == 402) {
        console.log(JSON.stringify(response));
      }
    });
...
edit(id){
  ...
  console.log("Professional Id---> ",this.professionalid);
  var url = `membership/getAProfessional?organizerId=`+this.userid._id+`&professionalId=`+this.professionalid;
  this.service.getApi(url,1).subscribe(response => {
    console.log(JSON.stringify(response));
   if(response.responseCode == 200) 
   {
     this.eachprofessionalData = response.result;
     this.editprofessionalphoto = [this.eachprofessionalData.imageURL] ;
     console.log("Data--> ",JSON.stringify(this.eachprofessionalData));
     $('#edit_professional').modal('show');
   } 
   else if(response.responseCode == 402) 
   {
...
editprofessionalFunc(formvalue){
  console.log("Form Value---> ",JSON.stringify(formvalue));
  var editdata = formvalue;
  var edited = {
    "organizerId":this.userid._id,
    "professionalName":editdata.professionalname,
    "professionalId":this.professionalid,
    "email":editdata.email,
    "countryCode":editdata.mobilecode,
    "mobileNumber":editdata.mobile,
    "status":editdata.status,
    "imageURL":this.edit64
    }
    
   console.log("Edit Member Data---> ",JSON.stringify(edited));
  this.service.postApi(`membership/editProfessional`,edited,1).subscribe(response => {
    if(response.responseCode == 200) {
      $('#edit_professional').modal('hide');
      this.editprofessionalForm.reset();
     this.professionalApiFunc('',this.filter.currPage);
    }  
    else if(response.responseCode == 402) {
      console.log(JSON.stringify(response));
...
deleteprofessionalApi(){
  var url =`membership/deleteProfessional?professionalId=`+this.professionalid;
  this.service.getApi(url,1).subscribe(response => {
    console.log(JSON.stringify(response));
