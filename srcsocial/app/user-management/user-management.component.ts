...
export class UserManagementComponent implements OnInit {
  ...
  broadcastMessage() {
    this.spinner.show();
    let data = {message:this.searchFormm.value.broadcast}
    this.service.postApii('admin/broadcast', data, 1).subscribe((res:any)=>{
      this.service.success("Broadcast message sent");
      this.searchFormm.reset();
      this.getUserList();
      this.spinner.hide();
    })
  }
  ...
  getUserList() {
    ...
    this.spinner.show();
    this.service.postApii('admin/listOfUser', data, 1).subscribe(success => {
      console.log("success", success);
      if (success.response_code == 200) {
        // this.service.success(success.response_message)
        this.copyUserList = success.result[0].docs;
        this.pagination.total = success.result[0].total;
        console.log("jdshfg87dsyfghjbdsjf",this.copyUserList, this.copyUserListLength)
        this.spinner.hide();
      }
      else {
        this.spinner.hide();
