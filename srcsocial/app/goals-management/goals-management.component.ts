...
@Component({
  ...
  styleUrls: ['./goals-management.component.css']
})
export class GoalsManagementComponent implements OnInit {
  ...
  goal() {
    this.spinner.show();
    this.service.postApii('admin/filterGoals', {categoryId:this.firstId}, 1).subscribe((res:any)=>{
      console.log("dkfjfghd98fdksjbg",res)
      if(res.response_code == 200) {
        this.copyUserList = res.result;
        this.spinner.hide();
      }
      else {
        this.spinner.hide();
  ...
  goall() {
    this.spinner.show();
    this.service.postApii('admin/filterGoals', {categoryId:this.searchForm.value.category}, 1).subscribe((res:any)=>{
      console.log("dkfjfghd98fdksjbg",res)
      if(res.response_code == 200) {
        this.copyUserList = res.result;
        this.spinner.hide();
  ...
  listOfCategory(){
    ...
    this.service.postApii('admin/listOfCategory', data,  1).subscribe(success => {
      ...
      if(success.response_code == 200) {
        ...
        this.firstId = success.result[0].docs[0]._id;
        this.goal();
        this.spinner.hide();
